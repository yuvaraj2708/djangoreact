...
var _debugger = require('./debugger.cjs.js');
require('rxjs');
...
function whereMyRjIsIvoked(wrappedComponentName) {
  ...
  for (var i = 0; i < stack.length; i++) {
    var _stack$i = _slicedToArray__default['default'](stack[i], 1),
    ...
    if (!startedMainHook && functionName === 'useMiniRedux') {
      ...
      if (rjHooks.indexOf(functionName) !== -1) {
        ...
        if (wrappedComponentName) {
          return {
            component: "<".concat(wrappedComponentName, " />"),
            hooks: hooks,
            rjFn: 'connectRj'
          };
...
function rjLogger() {
  ...
  _debugger.RjDebugEvents.subscribe(function (event) {
    if (event.type === _debugger.RJ_INIT_EVENT) {
      ...
      var rjName = info.name || "".concat(_index + 1, "\xB0");
      var component = location.component,
          rjFn = location.rjFn,
          hooks = location.hooks;
      var componentLocation = "%c".concat(component, "%c").concat(hooks.map(function (h) {
        return "  ".concat(h, "()");
      }).reverse().join(''), "  ").concat(rjFn, "(").concat(rjName, ")");
      ...
      console.groupCollapsed("".concat(componentLocation, "  %caction %c").concat(action.type), 'color: #80338a;font-weight:normal', "color:".concat(color, ";font-weight:normal"), 'color:grey;font-weight:lighter;', 'color:#464646;');
      console.log("%cprev state", 'color: grey;font-weight:normal;', prevState);
      console.log("%caction ".concat(pad(4)), 'color:deepskyblue;font-weight:normal;', action);
      ...
      var _rjName = _info.name || "".concat(_index2 + 1, "\xB0");

      var _component = _location.component,
          _rjFn = _location.rjFn,
          _hooks = _location.hooks;

      var _componentLocation = "".concat(_component).concat(_hooks.map(function (h) {
        return "  ".concat(h, "()");
      }), "  ").concat(_rjFn, "(").concat(_rjName, ")");

      var error = event.payload;
      console.error("[react-rocketjump] in ".concat(_componentLocation, "\n  ").concat(error));
    }
  });
}
